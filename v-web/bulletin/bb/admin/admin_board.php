...
else
{
	while( $row = $db->sql_fetchrow($result) )
	{
		...
		// Ckeck if style is being changed
                if(!strcmp($config_name,'default_style')) {
                        $old_style = $config_value;
                        if ($old_style != $HTTP_POST_VARS['default_style']) {
                                $sql = "SELECT template_name FROM " . THEMES_TABLE ." WHERE themes_id=".$HTTP_POST_VARS['default_style'];
                                $data = $db->sql_fetchrow($db->sql_query($sql));
                                $HTTP_POST_VARS['board_logo'] = "images/logo/".$data[0].".gif";
                        }
                }
...
$template->assign_vars(array(
	...
	"L_DEFAULT_LOGO" => $lang['Default_logo'],
        "L_DEFAULT_LOGO_EXPLAIN" => $lang['Default_logo_explain'],
