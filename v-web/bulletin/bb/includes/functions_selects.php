...
{
        global $db;
        $sql = "SELECT config_value FROM " . CONFIG_TABLE . " WHERE config_name='board_logo'";
        $logo_file = $db->sql_fetchrow($db->sql_query($sql));

        $logo_select = '<select name="board_logo">';

        $dir = "../images/logo";
        if (is_dir($dir)) {
                if ($dh = opendir($dir)) {
                        while (($file = readdir($dh)) !== false) {
                                $selected = ( !strcmp($logo_file[0],"images/logo/$file") ) ? ' selected="selected"' : '';
                                if ($file != '.' && $file != '..') {
                                        $logo_select .= '<option value="images/logo/' . $file . '"' . $selected . '>' .$file. '</option>';
                                }
                        }
                        closedir($dh);
                }
