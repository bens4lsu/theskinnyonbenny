...
function write2log ($linetoadd) {
	...
	$fplog=fopen('/home/users/web/b1051/sl.theskinn/public_html/lpoll/lp_log.dat', "a");
...
if (isset($votingstep)) {
	 function ShowTheStuff($item, $itemvoted, $graph_width, $graph_height) {
		...
		while ($in<$hector) {
			$stepstr=$stepstr.stripslashes($item[$in]).': '.($itemvoted[$in]).' ('.(int)($itemvoted[$in]*100/$totalvotes).'%)<br>';
			$timesred=(int)((($itemvoted[$in]/$totalvotes))*$graph_width);
			$stepstr=$stepstr.'<img height='.$graph_height.' width='.$timesred.' src="lpoll/lp_1.gif"><img height='.$graph_height.' width='.($graph_width-$timesred).' src="lp_0.gif"><br><br>';
			$in++;
		}
		return $stepstr;
	}
